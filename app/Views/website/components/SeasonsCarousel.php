<section class="mt-12 relative overflow-hidden" aria-label="carousel">
	...
	<div class="slides__container flex transition-all ease-in delay-200">
		<!-- It's important every slide has a layer with a classlist of "slide w-full shrink-0" -->
		<?php foreach ($seasons as $itr => $season) : ?>
			<div class="slide w-full shrink-0" role="group" aria-label={`slide ${index + 1}`}>
				<?= $this->setVar('season', $season)->include('website/components/Season') ?>
			</div>
		<?php endforeach ?>
	</div>
</section>
